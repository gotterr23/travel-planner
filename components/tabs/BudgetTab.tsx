  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-gradient-to-r from-blue-500 to-indigo-500 rounded-2xl p-5 text-white">
        <p className="text-blue-100 text-sm font-medium">총 지출</p>
        <p className="text-3xl font-bold mt-1">{total.toLocaleString()}원</p>
        <div className="flex flex-wrap gap-2 mt-3">
          {CATEGORIES.filter(c => byCategory[c]).map(c => (
            <span key={c} className="bg-white/20 rounded-full px-2.5 py-1 text-xs font-medium">
              {CATEGORY_EMOJI[c]} {c} {byCategory[c]!.toLocaleString()}원
            </span>
          ))}
        </div>
      </div>
      ...
      {showForm && (
        <div className="bg-white rounded-xl border border-slate-200 p-4 space-y-3">
          ...
          <input type="text" value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))}
            placeholder="항목 이름 (예: 도톤보리 타코야키)" className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" />
          <div className="grid grid-cols-2 gap-2">
            <input type="number" value={form.amount} onChange={e => setForm(f => ({ ...f, amount: e.target.value }))}
              placeholder="금액 (원)" className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" />
            <input type="text" value={form.paid_by} onChange={e => setForm(f => ({ ...f, paid_by: e.target.value }))}
              placeholder="결제자 (선택)" className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" />
          </div>
      ...
      {items.length === 0 ? (
        ...
        <div className="space-y-2">
          {items.map(item => (
            <div key={item.id} className="bg-white rounded-xl border border-slate-200 px-4 py-3 flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  ...
                  {item.paid_by && <span className="text-xs text-slate-400">{item.paid_by} 결제</span>}
                </div>
              </div>
              <div className="text-right shrink-0">
                <p className="font-semibold text-slate-800">{item.amount.toLocaleString()}원</p>
              </div>
              <button onClick={() => deleteItem(item.id)} className="text-slate-300 hover:text-red-400 text-sm shrink-0 ml-1">✕</button>
            </div>
          ))}
        </div>
      )}
